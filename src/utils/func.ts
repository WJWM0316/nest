...
import { isRealNum, valueIsError,error, getObjType,  ABCatNum} from './utils';
import { getdatabyselectionD } from './getdata';
...
//函数功能：比较或运算
function luckysheet_compareWith(...params:any[]) {
    a++
...
function luckysheet_getcelldata(txt) {
    ...
    if (val.length > 1) {
        ...
        sheetIndex = luckysheetfile.index;
        sheetdata = luckysheetfile.data;
...
function luckysheet_getValue(...params:number[]) {
...
function luckysheet_offset_check() {
    ...
    if (cellRow0 < 0 || cellRow1 >= global['sheet'].data.length || cellCol0 < 0 || cellCol1 >= global['sheet'].data.length) {
        return formula.error.r;
    }

    return getRangetxt(global['index'], {
