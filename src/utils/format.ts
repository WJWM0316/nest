import { isRealNum, valueIsError } from './utils';
    ...
    */
    ...
    function format(fmt, v : any, o) {
        ...
        if(!!sfmt.match(reg)){
            ...
            if( vlength> 4){
                if(vlength > 8){
                    var y : any =parseInt(v) / 100000000;  //亿
                    var w : any = parseInt(v.subtract(y*100000000)) / 10000; //万
                    var q : any = v.subtract(y*100000000 + w*10000); //千以后
                    if(acc != ""){
                        q = numeral(q).format(acc); //处理精确度
                    }
                    v = y + "亿" + w + "万" + q;
                }else{
                    var w : any = parseInt(v) / 10000; //万
                    var q : any = v.subtract(w*10000) //千以后
                    if(acc != ""){
                        q = numeral(q).format(acc); //处理精确度
                    }
                    v = w + "万" + q;
...
var unescapexml = (function() {
    ...
    return function unescapexml(text) {
        ...
        if (i == -1) return s.replace(encregex, function($$, $1) {
            return String.fromCharCode(parseInt($1, $$.indexOf("x") > -1 ? 16 : 10)) || $$;
